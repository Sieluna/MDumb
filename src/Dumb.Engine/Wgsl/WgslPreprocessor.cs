...
public static class WgslPreprocessor
{
        ...
        ImportResolver importResolver)
    {
        ...
        var entryDirectives = DirectiveParser.Parse(source);
        var allDefs = MergeDefs(shaderDefs, entryDirectives.Defines);

        // Build module graph
        var modules = ModuleGraph.BuildAndSort("main", source, importResolver, diagnostics);
        if (diagnostics.Any(d => d.Severity == DiagnosticSeverity.Error))
            return ProcessResult.Failure(diagnostics);

        // Combine into final WGSL
        var combined = SourceCombiner.Combine(modules, allDefs, diagnostics);

        return new ProcessResult { CombinedSource = combined, Diagnostics = diagnostics };
    }
        ...
        IReadOnlyDictionary<string, string>? shaderDefs)
    {
        ...
        var resolver = new FileSystemImportResolver(Path.GetDirectoryName(fullPath)!, fullPath);
...
internal sealed class FileSystemImportResolver
{
    ...
    public FileSystemImportResolver(string rootDir, string entryPath)
    {
        _rootDir = rootDir;
        ...
        try
        {
            foreach (var wgslFile in Directory.EnumerateFiles(_rootDir, "*.wgsl", SearchOption.AllDirectories))
            {
                try
                {
                    ...
                    var directives = DirectiveParser.Parse(content);
